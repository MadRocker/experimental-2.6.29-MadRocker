...
static int s5k5cagx_attach(struct i2c_adapter *adap, int addr, int kind)
{
	struct i2c_client *c;
	...
	strcpy(c->name, "s5k5ca");
	c->addr = addr;
	c->adapter = adap;
	c->driver = &s5k5ca_driver;
	s5k5ca_data.sensor = c;
...
static int s5k5ca_sensor_attach_adapter(struct i2c_adapter *adap)
{
	return i2c_probe(adap, &s5k5ca_addr_data, s5k5cagx_attach);
}

static int s5k5ca_sensor_detach(struct i2c_client *client)
{
...
static struct i2c_driver s5k5ca_driver = {
	...
	.id = S5K5CA_ID,
	.attach_adapter = s5k5ca_sensor_attach_adapter,
	.detach_client = s5k5ca_sensor_detach,
