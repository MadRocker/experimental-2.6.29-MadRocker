...
#if defined(CONFIG_S3C64XX_POWER_DOMAIN)
#include <linux/regulator/consumer.h>
#define USE_CAMERA_DOMAIN_GATING
static struct regulator *cam_domain = 0;
...
long s3c_camif_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case VIDIOC_S_WHITE_BALANCE:
		...
		break;
	case VIDIOC_S_FRAMERATE: //hjkang_DC11 (...who doesn't know that (1 << 32) in a 32-bit type equals 0... // tom3q)
		ctrl = (struct v4l2_control *) arg; 
...
#if 1	// for idle Current GPIO Setting
	s3c_camif_set_gpio(1);
#endif

#ifdef USE_CAMERA_DOMAIN_GATING
	regulator_disable(cam_domain);
#endif /* USE_CAMERA_DOMAIN_GATING */
...
static int s3c_camif_probe(struct platform_device *pdev)
{
...
#ifdef USE_CAMERA_DOMAIN_GATING
	cam_domain = regulator_get(&pdev->dev, "pd");
	if(IS_ERR(cam_domain)) {
		printk("Failed to get camera power domain.\n");
		return -1;
	}
	regulator_enable(cam_domain);
#endif /* USE_CAMERA_DOMAIN_GATING */
	...
	/* ioremap for lcd controller */
	codec->lcd_regs = preview->lcd_regs = ioremap(S3C64XX_PA_FB, SZ_16K);

	if (codec->lcd_regs == NULL) {
		printk(KERN_ERR "Failed to remap lcd register block\n");
		return -1;
	}
	...
	cam_hclk = clk_get(&pdev->dev, "camera_hclk");
